...
input:y——观测值，k——用于计数，axis——x为0、y为1、z为2
...
float Model::CSM_predict(float y,int k,int axis)

{
	...
	velocity=X_P[(k+2)%3](1,0);

	acceleration=X_P[(k+2)%3](2,0);

	//predict=X_P[(k+2)%3](0,0);

	//cout << " v:" << velocity << " a:" << acceleration << endl;

	return amend(y,k,axis);
}



float Model::getMin(float a,float b,float c)
...
float Model::getMax(float a,float b,float c)
...
input: reality——观测值，predict——预测值，k——用于计数，axis——x为0、y为1、z为2

*/

float Model::amend(float reality,int k,int axis)

{

	float average, variance, predict;

	average = (X[(k + 1) % 3](1, 0) + X[k](1, 0) + velocity) / 3.0;//均值

	variance = abs(X[(k + 1) % 3](1, 0) - average) + abs(X[k](1, 0) - average) + abs(velocity - average);//

	cout << "aver:" << average << " variance:" << variance << endl;
...

	{
		...
		{
			float sign=velocity / velocity;

			velocity = sign * abs(getMin(X[(k+1)%3](1,0),X[k](1,0),velocity))/2.0;

			acceleration = 0;

		}

		if (abs(velocity) < 1 || (abs(X[(k + 1) % 3](1, 0)) < 1 && abs(X[k](1, 0) < 1)))
		...
		//if (abs(velocity) < 8)
			...
			predict = reality + velocity * 15 + 15 * 15 * acceleration / 2.0;

		//else if (velocity >0)

		//	predict = reality + 50.0;
...

	{
		...
		if (abs(velocity) < 2 || (abs(X[(k + 1) % 3](1, 0)) < 2 && abs(X[k](1, 0)) < 2)) 
		...
		//if (abs(velocity) < 8)
			...
			predict = reality + velocity * 7 + 7 * 7 * acceleration / 2.0;
	...
	if(abs(velocity)<10)
		...
		nextframe=reality+velocity+acceleration/2;

	else if(velocity>0)

		nextframe=reality+10;
...
input:isReset——初始化模型或连续三帧跟丢后，再次识别到重置模型，isReset为1
...
{
	cout<<"current---"<<"pitch:"<<pitch<<"   "<<"yaw:"<<yaw<<endl;
...
//-------------------------运动模型----------------------
	...
	x_pred=tracking_x.CSM_predict(x_,times,0);

	z_pred=tracking_z.CSM_predict(z_,times,2);
...
//-------------------------运动模型----------------------
	...
	x_pred=tracking_x.CSM_predict(last_pred[0],times,0);

	z_pred=tracking_z.CSM_predict(last_pred[2],times,2);
