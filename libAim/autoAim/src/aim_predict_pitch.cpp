#include"aim_predict_pitch.h"
using namespace cv;
Aim_predict_pitch::Aim_predict_pitch(){}
Aim_predict_pitch::~Aim_predict_pitch(){}

void Aim_predict_pitch::modelInit(){
...
void Aim_predict_pitch::resetKfStatepost(Mat statepost){
    kalman_fileter_.setStatepost(statepost);
}

Mat Aim_predict_pitch::predict(Mat measurement, float dt){
    kalman_fileter_.setSamplingtime(dt);
    kalman_fileter_.correct(measurement);
    return kalman_fileter_.predict();
}
void Aim_predict_pitch::clear(){
    angle_velocity_.clear();
    yaw_.clear();
    predict_angle_.clear();
}
bool Aim_predict_pitch::shootLogic(float initYaw, float angel_velocity, float predict_angle){
