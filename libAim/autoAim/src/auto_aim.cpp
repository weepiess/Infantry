...
Point2d AutoAim::cal_x_y_sp(RotatedRect &rect, int is_up){
    float angle = (90-rect.angle)*CV_PI/180;
    Point2d point;
    if(is_up){
...
bool AutoAim::setImage(Mat &img){
    ...
    // int thresh =200 , substract_thresh = 100;//二值化阈值
    // resetROI();
    // mask = img(rectROI);
    // split(mask, channel);//分离rgb通道
    // Mask = channel[1];//绿通道获取目标颜色及高亮部分(灯条形状完整)
    // mask = channel[0] - channel[2];//获取目标颜色部分(灯条形状残缺但无噪声)
    // threshold(Mask, Mask, thresh, 255, THRESH_BINARY);
    // threshold(mask, mask, substract_thresh, 255, THRESH_BINARY);
    // Mat element = getStructuringElement( MORPH_ELLIPSE, Size(5, 5));
    // Mat element2 = getStructuringElement( MORPH_ELLIPSE, Size(3, 7));
    // morphologyEx(mask,mask,MORPH_CLOSE,element);
    // Mat element1 = getStructuringElement( MORPH_ELLIPSE, Size(9, 5),Point(4,4));
    // dilate( mask, mask, element1,Point(-1,-1));
    // bitwise_and(Mask, mask, mask);//与运算获取完整灯条形状并去除噪声干扰
    ...
    // dilate( mask, mask, element1,Point(-1,-1));
    // dilate( Mask, Mask, element,Point(-1,-1),1);
    imshow("MM",Mask);
    imshow("mm",mask);
    //imshow("img1",Mask);
    ...
    // //msrcr.MultiScaleRetinexCR(image, image, weight, sigema, 128, 128);
    // Mat channel[3], Mask, diff;
    // int thresh =130 , substract_thresh = 150;
    // resetROI();
    // mask = img(rectROI);
    // split(mask, channel);
    // Mask = channel[1];
    ...
    if(LIKELY(contours.size()<200)){
        for(int i=0;i<contours.size();i++){
            if(contours[i].size()>5){
                ...
                // if(abs(temp.size.area()<50)) continue;//细小噪声滤除
...
void AutoAim::match_lamps(vector<RotatedRect> &pre_armor_lamps, vector<RotatedRect> &real_armor_lamps){
    ...
    vector<float> diff_sp(size,0x3f3f3f3f);
    vector<float> best_match_index_sp(size,-1);
    //计算灯管匹配之间的花费
    ...
    for(i=0; i<size; i++){
        ...
        for(j=i+1;j<size; j++){
            ...
            // if(current.center.y < 300) continue;
            //灯条角度差超过设定角度忽略
            cout<<"diff_ang:  "<<fabsf(theta_compare - theta_current)<<endl;
            ...
            cout<<"yx_ratio: "<< fabsf(current.center.y-compare.center.y)/fabsf(current.center.x-compare.center.x)<<endl;
...
void AutoAim::select_armor(vector<RotatedRect> real_armor_lamps){
    ...
    if(special_condition){
        ...
        for(int i=0; i<real_armor_lamps.size(); i+=2){
            ...
            int y = (left_up.y + right_up.y) / 2 - 0.5 * real_armor_lamps[i].size.height;
            if(y<0) armor_area.y;
            else armor_area.y = y;
            if(left_d.y>right_d.y){
                if((1.8) * (left_d.y - right_up.y)+armor_area.y<720)
                    armor_area.height = (1.8) * (left_d.y - right_up.y);
                else{
                    armor_area.height = 720 - armor_area.y;
                }
            }else{
                if((1.8) * (right_d.y - left_up.y)+armor_area.y<720)
                    armor_area.height = (1.8) * (right_d.y - left_up.y);
                else{
                    armor_area.height = 720 - armor_area.y;
                }
            }    
            ...
            if(armor_area.width != 0 && armor_area.height != 0 ){
                ...
                imwrite("../armor_data/3/"+to_string(c)+"2000.png",img_a);
                #endif
            }
            ...
            if((float(x0)/float(y0))>0.15 && (float(x0)/float(y0))<0.39 
                && (float(x0)/float(real_armor_lamps[i].size.height)) > 3.2 
                && (float(x0)/float(real_armor_lamps[i].size.height) < 4.08)){
                detect_hero = true;
            }
        }
        // imshow("image",image);
        ...
        for(int i=0;i<armor_detected.size();i++){
            if(fire && armor_detected[i] == fire_id){
                //todo: 接收英雄车指令,优先击打英雄车击打目标,帮助英雄完成击杀
            }
            if((armor_detected[i] == 2 || detect_hero) && (armor_detected[i] == 1)){
                pnpSolver.clearPoints3D();
            ...
            //不瞄准工程车
            else if(armor_detected[i] != 10 && armor_detected[i] != 10 && armor_detected[i]!=10){
    ...
    if(special_condition){
        ...
        bestCenter.x = real_armor_lamps[sp_index].center.x - 1.2 * real_armor_lamps[sp_index].size.height + rectROI.x;
        bestCenter.y = real_armor_lamps[sp_index].center.y + rectROI.y - 0.1*real_armor_lamps[sp_index].size.height;   
    }
    if(hero_index!=-1){
        resizeCount=0;
        count++;
...
BaseAim::AimResult AutoAim::aim(Mat &src, float currPitch, float currYaw, Point2f &pitYaw,int is_predict,bool &if_shoot,float time_delay){
    ...
    if(bestCenter.x!=-1){
        circle(src,bestCenter,10,Scalar(0,255,0),3);
        if(!special_condition){
            count++;
            Mat frame;
            src.copyTo(frame);
