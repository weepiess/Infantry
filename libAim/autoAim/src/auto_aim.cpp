...
bool AutoAim::setImage(Mat &img){
    ...
    //      if(img.empty()) return false;
    //  img.copyTo(image);
    // int thresh = 90, substract_thresh = 100;
    // Mat channel[3], gray,flood2;
    // cv::split(img, channel);
    // gray = channel[1];
    // threshold(gray, gray,thresh, 255, cv::THRESH_BINARY);
    // Mat diff, Mask;
    // diff = channel[0] - channel[2];
    // threshold(diff,Mask,substract_thresh,255,THRESH_BINARY);
    // Mat element = getStructuringElement(cv::MORPH_ELLIPSE, cv::Size(9, 9), cv::Point(4, 4));
    // Mat element2 = getStructuringElement(MORPH_ELLIPSE,Size(1,3));
    // dilate(Mask, Mask, element,Point(-1,-1),1);
    // // Mat flood = Mask.clone();
    // // floodFill(flood, Point(0, 0), Scalar(255));
    // // bitwise_not(flood, flood);
    // // bitwise_or(Mask,flood,Mask);
    // //dilate(gray,gray,element2,Point(-1,-1),1);
    // blur(gray,gray,Size(2,2));
    // threshold(gray,gray,90,255,THRESH_BINARY);
    // bitwise_and(Mask, gray, mask);
    // blur(mask,mask,Size(2,2));
...
void AutoAim::findLamp_rect(vector<RotatedRect> &pre_armor_lamps){
    ...
    if(LIKELY(contours.size()<200)){
        ...
        //         y_down = 0;
        //         temp = adjustRRect(fitEllipseDirect(contours[i]));
        //         minEnclosingCircle(contours[i],cent,radius);
        //         cout<<"r  "<<radius<<endl;
        //         // circle(image,cent,radius,Scalar(0,255,0),1);
        //         // circle(image,cent,1,Scalar(0,0,0),2);
        //         for(int j=0; j<contours[i].size(); j++){
        //             if(fabsf(sqrtf(powf((contours[i][j].y - cent.y),2)+powf((contours[i][j].x - cent.x),2)) - radius) < 0.001){
        //                 if(contours[i][j].y < cent.y){
        //                 y_up = contours[i][j].y;
        //                 y_up_x = contours[i][j].x;
        //                 }
        //                 if(contours[i][j].y > cent.y){
        //                 y_down = contours[i][j].y;
        //                 y_down_x = contours[i][j].x;        
        //                 // cout<<"x "<<y_down_x<<" y "<<y_down<<endl;       
        //                 // cout<<"c  x "<<cent.x<<"  c y  "<<cent.y<<endl;
        //                 // cout<<" r "<<radius<<endl;             
        //                 }
        //                 // if(contours[i][j].y > y_down){
        //                 //     y_down = contours[i][j].y;
        //                 //     y_down_x = contours[i][j].x;
        //                 // }
        //                 // if(contours[i][j].y < y_up){
        //                 //     y_up = contours[i][j].y;
        //                 //     y_up_x = contours[i][j].x;
        //                 // }
        //             }
        //         }
        //             // circle(image,Point(y_up_x,y_up),2,Scalar(0,0,255),2);
        //             // circle(image,Point(y_down_x,y_down),2,Scalar(0,0,255),2);
        //             // imshow("ima",image);
        //             //waitKey(1);
        //         temp.angle = atanf((y_down - y_up)/(y_up_x - y_down_x))*180/CV_PI ;
        //         if(temp.angle >= 0)
        //             temp.angle = fabsf(temp.angle - 90);
        //         if(temp.angle < 0)
...
void AutoAim::match_lamps(vector<RotatedRect> &pre_armor_lamps, vector<RotatedRect> &real_armor_lamps){
    ...
    for(i=0; i<size; i++){
        ...
        for(j=i+1;j<size; j++){
            ...
            if(current.center.y < 360) continue;
            //灯条角度差超过设定角度忽略
...
BaseAim::AimResult AutoAim::aim(Mat &src, float currPitch, float currYaw, Point2f &pitYaw,int is_predict,bool &if_shoot,float time_delay){
    ...
    if(bestCenter.x!=-1){
        ...
        if(is_predict && isKalman){
            ...
            // ofstream outfile3("/home/weepies/output3.txt", ios::app);
            if(condition!=cod::CONDITION_NOR){
                if(condition == cod::CONDITION_ABN){  //正常模式
                }
                if(condition == cod::CONDITION_NOR_WIGGLE){ //正常扭腰
                    k_add = 0.15; //低抑制低通滤波
                    if(predict_angle>0.6)
                        predict_angle = 0;
                    cout<<"** NOR_WIGGLE **  ************************************"<<endl;
                }
                if(condition == cod::CONDITION_FAST_WIGGLE){ //快速扭腰
                    k_add = 0.14; //高抑制低通滤波
                    if(predict_angle>0.6)
                        predict_angle = 0;
            ...
            // outfile3<<pitYaw.y<<endl;
            new_number = pitYaw.y; //更新扭腰判断数据输入
            if(condition != cod::CONDITION_NOR && condition!= cod::CONDITION_ABN){ //扭腰模式下低通滤波
